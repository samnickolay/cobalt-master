...
class EventSimulator(Component):
    ...
    # dwang
        ...
        if self.go_next:
            ...
            utilization_chunk = (old_time, new_time, current_utilization)

            from bqsim import utilization_records
            global utilization_records

            if old_time > 0.0:
                utilization_records.append(utilization_chunk)
...
#
        if machine == INTREPID:
            ...
            # samnickolay
                ...
                self.bgsched.schedule_jobs_wcheckp_v2_sam_v1(preempt, fp_backf, fp_pre_bj,
                                                             checkp_dsize, checkp_w_bandwidth, checkp_r_bandwidth,
                                                             checkp_heur_opt, job_length_type)

            elif checkpoint_opt == "v2p_sam_v1":
                self.bgsched.schedule_jobs_wcheckp_v2p_sam_v1(preempt,fp_backf,fp_pre_bj, checkp_dsize, checkp_w_bandwidth,
                                                          checkp_r_bandwidth, checkp_t_internval, checkp_heur_opt,
                                                              job_length_type)
            elif checkpoint_opt == "v2p_app_sam_v1":
                self.bgsched.schedule_jobs_wcheckp_v2p_app_sam_v1(preempt, fp_backf, fp_pre_bj, checkp_dsize,
